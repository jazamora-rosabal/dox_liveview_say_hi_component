defmodule SayHiComponentWeb.CalendarMonthYear do
  ...
  def render(assigns) do
    ...
    <div
      ...
      phx-value-block="<%= is_block?(@date, @min_date, @max_date, @picker_mode, @calendar_mode, @calendar) %>"
      class="w-1/3 text-sm py-4 px-8 lowercase text-center border <%= @column_class %>"
    >
  ...
  defp after_max_date?(assigns) do
    after_max_date?(assigns.picker_mode, assigns.calendar_mode, assigns.calendar, assigns.date, assigns.max_date)
  end

  defp after_max_date?(picker_mode, calendar_mode, calendar, date, max_date ) do
    if max_date !== nil, do: Helper.after_max_date?(picker_mode, calendar_mode, calendar, date, max_date), else: false
  end


  defp before_min_date?(assigns) do
    before_min_date?(assigns.picker_mode, assigns.calendar_mode, assigns.calendar, assigns.date, assigns.min_date)
  end

  def before_min_date?(picker_mode, calendar_mode, calendar, date, min_date) do
    if min_date != nil, do: Helper.before_min_date?(picker_mode, calendar_mode, calendar, date, min_date), else: false
  end
  ...
  defp is_block?(date, min_date, max_date, picker_mode, calendar_mode, calendar) do
    picker_mode |> before_min_date?(calendar_mode, calendar, date, min_date) || picker_mode |> after_max_date?(calendar_mode, calendar, date, max_date )
  end
